...
import cn.com.leador.mapapi.tracker.track.input.TrackColumnListInput;
import cn.com.leador.mapapi.tracker.track.output.TrackColumnListOutput;
@WebController
public class TrackColumnListController extends HttpServletProxy {
	private Logger logger = LogManager.getLogger(this.getClass());
	
	private CommonInputProxy<List<TrackColumnBean>> input=null;
	private CommonOutputProxy<List<TrackColumnBean>> trans=null;
	@PostConstruct
	private void init(){
		if(logger.isDebugEnabled()){
			logger.debug("初始化"+this.getClass());
		}
		input=SpringContextHelper.getBeanByType(TrackColumnListInput.class);
		trans=SpringContextHelper.getBeanByType(TrackColumnListOutput.class);
	}

	@Override
	@UriMapping(value="/track/listcolumn",sid="100007",name="track自定义字段列表")
	public void commonProceed(HttpServletRequest request,
			HttpServletResponse response, ClientInfo clientInfo) {
		ResultBean<List<TrackColumnBean>> bean=null;
		try {
