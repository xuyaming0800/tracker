...
public class EntityColumnAddInput extends CommonInputProxy<EntityColumnBean> {
	...
	@Override
	protected ResultBean<EntityColumnBean> getSearchMethod(ClientInfo clientInfo,
			HttpServletRequest request) throws Exception {
		Map<String, String> map = clientInfo.getInfoTable();
		EntityColumnBean bean=new EntityColumnBean();
		bean.setAk(map.get("ak"));
		bean.setColumn_desc(map.get("column_desc")==null?"":map.get("column_desc"));
		...
		ResultBean<EntityColumnBean> result=entityColumnService.addColumn(bean);
		return result;
	}
