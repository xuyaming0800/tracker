...
@Component
public class FenceCreateInput extends CommonInputProxy<FenceBean> {
...
//		ResultBean<TrackBean> result = trackService.addPoint(bean);
		return fenceService.createFence(bean);
	}
	...
	protected BusinessExceptionBean checkInput(ClientInfo clientInfo,
		...
		if (this.checkParamIsNull(creator)) {
			bean.appnedMessage("creator不能为空");
			return bean;
		}
		_bean.setCreator(creator);
		...
		if (this.checkParamIsNull(valid_times)) {
			bean.appnedMessage("valid_times不能为空");
			return bean;
		}
		...
		if (this.checkParamIsNull(valid_cycle)) {
			bean.appnedMessage("valid_cycle不能为空");
			return bean;
		}
		...
		if (this.checkParamIsNull(alarm_condition)) {
			alarm_condition=String.valueOf(FENCE_ALARM_CONDITION_TYPE.ALL.getType());
		}
